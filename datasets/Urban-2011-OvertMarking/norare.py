...
def download(dataset):
    ...
    print("data is static and cannot be downloaded")
...
def map(dataset, concepticon, mappings):
    ...
    urban = models.Conceptlist.from_file(
            dataset.raw_dir / "Urban-2011-160.tsv"
            )
    ...
    # get indo-aryan shifts, we correct errors here
    reps = {"mirrow": "mirror", "straw/hay": "straw", "cheeck": "cheek"}
    iashifts = collections.defaultdict(dict)
    for row in dataset.get_csv("indo-aryan-shifts.tsv", delimiter='\t', dicts=True):
